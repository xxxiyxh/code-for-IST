...
def apfd(test_results):
    ...
    metric_value = 1 - ((sum(failure_order))/1.2 / (number_failure * number_tests)) + 1 / (2 * number_tests)
    return metric_value
...
for _ in range(num_iterations):
    selected_data = read_csv_and_select_data('Weighted Adder (part1).csv', m=600, epsilon=epsilon, delta=delta)
