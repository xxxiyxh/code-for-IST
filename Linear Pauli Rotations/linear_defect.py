...
class LinearPauliRotations_defect(FunctionalPauliRotations):
    ...
    def _build(self):
        ...
        for i, q_i in enumerate(qr_state[1:]):                          # decrease one
            if self.basis == "x":
                circuit.crx(self.slope * pow(2, i), q_i, qr_target)
            elif self.basis == "y":
                circuit.cry(self.slope * pow(2, i), q_i, qr_target)     
            else:  # 'Z'
                circuit.crz(self.slope * pow(2, i), qr_target, q_i)     # switch
